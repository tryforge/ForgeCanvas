...
exports.CanvasUtil = {
    validateFont: (font) => {
        ...
        if (!match?.[0])
            return FCError.InvalidFontFormat;
    ...
    resolveFrame: async (self, ctx, frame, speed) => {
        switch (frame.substring(0, frame.indexOf(':'))) {
            ...
            case 'canvas': {
                ...
                if (!canvas)
                    return self.customError(FCError.NoCanvas);
                return gifsx_1.Frame.fromRgba(canvas.width, canvas.height, Uint8Array.from(canvas.canvas.data()), speed);
            }
    ...
    parseText: async (client, text, parseNL, parseEmoji) => {
        if (!text?.trim()?.length)
            return [];
        ...
        while (i < text.length) {
            ...
            if (esc)
                ...
                const match = text.slice(i).match(exports.emojiRegex);
                if (match) {
                    const id = match[3];
                    let emoji = client.preloadImages.get(`emoji_${id}`);
                    if (!emoji) {
                        emoji = await (0, canvas_1.loadImage)(`https://cdn.discordapp.com/emojis/${id}.png`);
...
(function (FCError) {
    ...
    FCError["InvalidFontFormat"] = "Invalid font format provided";
    FCError["InvalidFont"] = "Provided font does not exist";
